...
dbmock: # Generates the DB mocks
	mockgen -package mockdb -destination db/mock/querier.go  github.com/ejozz/go-notes/db/sqlc Querier
.PHONY: dbmock

notesvcmock: # Generates the noteservice mocks
	mockgen -package mocknote -destination note/mock/notemock.go  github.com/ejozz/go-notes/server/http NoteService
.PHONY: notemock

build-backend: # Builds the backend Docker image
	docker build . -t go-notes-backend
.PHONY: build-backend

build-frontend: # Builds the frontend Docker image
	docker build ./web/ -t go-notes-frontend
.PHONY: build-frontend

run-backend: # Runs the backend Docker image
	docker run -d -p 8080:8080 go-notes-backend
.PHONY: run-backend

run-frontend: # Runs the frontend Docker image
	docker run -d -p 3000:3000 go-notes-frontend
.PHONY: run-frontend
